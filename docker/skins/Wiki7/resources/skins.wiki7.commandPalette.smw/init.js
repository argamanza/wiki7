...
function makeState( key ) {
	return {
		title: mw.message( 'wiki7-command-palette-mode-smw-' + key + '-title' ).text(),
		description: mw.message( 'wiki7-command-palette-mode-smw-' + key + '-description' ).text(),
...
function adaptSmwResult( subject, index ) {
	const item = {
		id: 'wiki7-command-palette-item-smw-' + index,
		type: 'smw',
	...
	return function ( fragment ) {
		return new mw.Api().get( {
			...
			return items.map( ( item, index ) => ( {
				id: 'wiki7-command-palette-item-' + typePrefix + '-' + index,
				type: typePrefix,
				thumbnailIcon: icon,
				label: item.label,
				highlightQuery: true
...
function fetchValueSuggestions( fragment, property ) {
	return new mw.Api().get( {
		...
		return values.map( ( value, index ) => ( {
			id: 'wiki7-command-palette-item-smw-value-' + index,
			type: 'smw-value',
		...
		if ( incomplete.stage === 'property' ) {
			return fetchPropertySuggestions( incomplete.fragment ).then( ( items ) => {
				...
				if ( 'category'.startsWith( incomplete.fragment.toLowerCase() ) ) {
					return [
						{
							id: 'wiki7-command-palette-item-smw-namespace-category',
							type: 'smw-namespace',
...
module.exports = {
	...
	icon: cdxIconWikitext,
	compactResults: true,
	label: mw.message( 'wiki7-command-palette-command-smw-label' ).text(),
	description: mw.message( 'wiki7-command-palette-command-smw-description' ).text(),
	placeholder: mw.message( 'wiki7-command-palette-mode-smw-placeholder' ).text(),
	...
	help: {
		description: 'wiki7-command-palette-mode-smw-description-help'
	},
