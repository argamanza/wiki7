 ...
 * `#wiki7-notifications-content` (replacing the server-rendered skeleton),
...
function createNotifications( { document, mw } ) {
	function init() {
		const details = document.getElementById( 'wiki7-notifications-details' );
		if ( !details ) {
			return;
		}

		const summary = details.querySelector( 'summary' );
		const contentEl = document.getElementById( 'wiki7-notifications-content' );
		if ( !summary || !contentEl ) {
			return;
		}

		const skeletonEl = contentEl.querySelector( '.wiki7-notifications__skeleton' );
		const errorEl = contentEl.querySelector( '.wiki7-notifications__error' );
		const retryBtn = errorEl ?
			errorEl.querySelector( '.wiki7-notifications__retry' ) :
			null;
