const
	STICKY_HEADER_ID = 'wiki7-sticky-header',
	STICKY_HEADER_VISIBLE_CLASS = 'wiki7-sticky-header-visible';
...
function prepareMenuDropdown( menuDropdown ) {
	const
		...
		menuDropdownButton = menuDropdownClone.querySelector( '.wiki7-dropdown-summary' );

	menuDropdownStickyElementsWithIds.forEach( ( stickyElement ) => {
		// Set up the click target to keep JS click handlers working (#1100)
		stickyElement.setAttribute( 'data-mw-wiki7-click-target', `#${ stickyElement.id } > a` );
		// Remove the id attribute to prevent duplicate ids
		stickyElement.removeAttribute( 'id' );
	} );

	// Sticky header buttons use Codex large size (44px) instead of Wiki7's (40px)
	menuDropdownButton.classList.remove( 'wiki7-cdx-button--size-large' );
...
class StickyHeader {
	...
	getClickTarget( fakeButton ) {
		return this.document.querySelector( fakeButton.getAttribute( 'data-mw-wiki7-click-target' ) );
	}
	 ...
	 */
	handleClick( event ) {
		const fakeButton = event.target.closest( '[data-mw-wiki7-click-target]' );
		if ( fakeButton ) {
	 ...
	 */
	initFakeButtons() {
		const fakeButtons = this.stickyHeaderElement.querySelectorAll( '.cdx-button[data-mw-wiki7-click-target]' );
		...
		fakeButtons.forEach( ( fakeButton ) => {
			...
			fakeButton.classList.add( 'wiki7-sticky-header-fake-button' );
		} );
	}
	...
	updateEditIcon() {
		...
		if ( sourceEditButton && !visualEditButton ) {
			const icon = sourceEditButton.querySelector( '.wiki7-ui-icon' );
			if ( icon ) {
	...
	initDropdowns() {
		const
			moreMenuDropdown = this.document.getElementById( 'wiki7-page-more-dropdown' ),
			moreMenuDropdownContainer = this.document.getElementById( 'wiki7-sticky-header-more' ),
			languagesDropdown = this.document.getElementById( 'wiki7-page-languages-dropdown' ),
			languagesDropdownContainer = this.document.getElementById( 'wiki7-sticky-header-languages' );
