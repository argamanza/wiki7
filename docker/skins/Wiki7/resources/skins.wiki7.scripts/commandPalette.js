const MODULE = 'skins.wiki7.commandPalette';
...
function createCommandPalette( { document, mw } ) {
	...
	function ensureOverlay() {
		...
		overlay.id = 'wiki7-command-palette-overlay';
		overlay.className = 'wiki7-command-palette-overlay';
	...
	function load() {
		...
		Promise.race( [ mw.loader.using( MODULE ), timeoutPromise ] ).then(
			...
			() => {
				...
				setOpenState( false );
				mw.notify(
					mw.message( 'wiki7-command-palette-load-error' ).text(),
					{ type: 'error' }
				);
			}
		);
	...
	function init() {
		const summary = document.getElementById( 'wiki7-search-summary' );
		if ( !summary ) {
			return;
		}
		detailsEl = document.getElementById( 'wiki7-search-details' );

		// `templates/Search.mustache` renders a `<div id="wiki7-search__card">`
		// as a no-JS fallback search form (revealed by Search.less when the
		// `<details>` opens), and `Search__button.mustache` carries an
		// `aria-details="wiki7-search__card"` attribute pointing at it.
		...
		const oldCard = document.getElementById( 'wiki7-search__card' );
		if ( oldCard ) {
			oldCard.remove();
		}
