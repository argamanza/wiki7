 ...
 * `<dialog>` with a skeleton on click, and lazy-loads `skins.wiki7.share`.
...
function createShare( { document, window, mw, navigator, mode = 'auto' } ) {
	...
	function init() {
		const trigger = document.getElementById( 'wiki7-share' );
		if ( !trigger ) {
			return;
		}
		...
		const mountPoint = needsPanel ?
			document.getElementById( 'wiki7-share-dialog-content' ) :
			null;
