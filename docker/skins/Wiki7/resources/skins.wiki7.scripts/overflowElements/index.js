...
class OverflowElement {
	...
	onClick( event ) {
		...
		} else if ( target.classList.contains( 'wiki7-overflow-navButton-right' ) ) {
			this.scrollContent( offset );
		}
	}
	...
	init() {
		const refs = createOverflowWrapper( {
			...
			inheritedClasses: this.config.wgWiki7OverflowInheritedClasses
		} );
		if ( !refs ) {
			return;
		}
		...
		const headerRow = this.element.querySelector( '.wiki7-overflow-sticky-header' );
