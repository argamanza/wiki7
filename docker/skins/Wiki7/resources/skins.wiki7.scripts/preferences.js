const MODULE = 'skins.wiki7.preferences';
...
function createPreferences( { document, mw } ) {
	 ...
	 * inside #wiki7-preferences-content; Vue's mount() replaces them when
	...
	function init() {
		...
		const contentEl = document.getElementById( 'wiki7-preferences-content' );
		if ( !summary || !contentEl ) {
			return;
		}

		const skeletonEl = contentEl.querySelector( '.wiki7-preferences-skeleton' );
		const errorEl = contentEl.querySelector( '.wiki7-preferences-error' );
		const retryBtn = errorEl ?
			errorEl.querySelector( '.wiki7-preferences-error__retry' ) :
			null;
