...
/**
 * Bind external `.wiki7-search-trigger` links (with optional
 * `data-wiki7-search-prefill`) to open the palette with optional
 * prefill text.
 *
...
function bindSearchTrigger( document, triggerOpen ) {
	document.querySelectorAll( '.wiki7-search-trigger' ).forEach( ( trigger ) => {
		trigger.addEventListener( 'click', () => {
			triggerOpen( trigger.dataset.wiki7SearchPrefill || null );
		} );
	} );
}
