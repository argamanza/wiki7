...
function createUserCommand( ApiConstructor ) {
	...
	function adaptUserResult( apiUser ) {
		return {
			id: 'wiki7-command-palette-item-user-' + apiUser.userid,
	...
	return defineMode( {
		id: 'user',
		triggers: [ '/user:', '@' ],
		label: mw.message( 'wiki7-command-palette-command-user-label' ).text(),
		description: mw.message( 'wiki7-command-palette-command-user-description' ).text(),
		placeholder: mw.message( 'wiki7-command-palette-mode-user-placeholder' ).text(),
		icon: cdxIconUserAvatar,
		compactResults: true,
		emptyState: {
			title: mw.message( 'wiki7-command-palette-mode-user-empty-title' ).text(),
			description: mw.message( 'wiki7-command-palette-mode-user-empty-description' ).text(),
			icon: cdxIconUserAvatar
		},
		noResults() {
			return {
				title: mw.message( 'wiki7-command-palette-mode-user-noresults-title' ).text(),
				description: mw.message( 'wiki7-command-palette-mode-user-noresults-description' ).text(),
				icon: cdxIconUserAvatar
			};
		},
		help: {
			description: 'wiki7-command-palette-mode-user-description-help'
		},
