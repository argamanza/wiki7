...
function createActionCommand( documentRef, ApiConstructor ) {
	...
	function fetchMenuItems() {
		...
		TARGET_PORTLET_IDS.forEach( ( portletId ) => {
			...
			const menuLabel = portletElement
				.querySelector( '.wiki7-menu__heading' )?.textContent?.trim();
			const links = portletElement
				.querySelectorAll( '.wiki7-menu__content-list li a' );
	...
	return defineMode( {
		id: 'action',
		triggers: [ '/action:', '>' ],
		label: mw.message( 'wiki7-command-palette-command-action-label' ).text(),
		description: mw.message( 'wiki7-command-palette-command-action-description' ).text(),
		placeholder: mw.message( 'wiki7-command-palette-mode-action-placeholder' ).text(),
		icon: cdxIconSpecialPages,
		compactResults: true,
		help: {
			description: 'wiki7-command-palette-mode-action-description-help'
		},
