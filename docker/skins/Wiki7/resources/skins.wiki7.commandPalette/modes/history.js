...
function adaptRevisionItem( rev, parentRev, title, tagDisplayNames ) {
	...
	const summary = text.trim() !== '' ?
		text :
		mw.message( 'wiki7-command-palette-mode-history-no-summary-text' ).text();

	return {
		id: 'wiki7-command-palette-item-revision-' + rev.revid,
...
function createHistoryMode( ApiConstructor ) {
	...
	return defineMode( {
		id: 'history',
		triggers: [ '/hist:', '!' ],
		label: mw.message( 'wiki7-command-palette-command-history-label' ).text(),
		description: mw.message( 'wiki7-command-palette-command-history-description' ).text(),
		placeholder: mw.message( 'wiki7-command-palette-mode-history-placeholder' ).text(),
		icon: cdxIconHistory,
		compactResults: true,
		emptyState: {
			title: mw.message( 'wiki7-command-palette-mode-history-empty-title' ).text(),
			description: mw.message( 'wiki7-command-palette-mode-history-empty-description' ).text(),
			icon: cdxIconHistory
		},
		noResults() {
			return {
				title: mw.message( 'wiki7-command-palette-mode-history-noresults-title' ).text(),
				description: mw.message( 'wiki7-command-palette-mode-history-noresults-description' ).text(),
				icon: cdxIconHistory
			};
		},
		help: {
			description: 'wiki7-command-palette-mode-history-description-help'
		},
		getResults,
		onResultSelect
	} );
