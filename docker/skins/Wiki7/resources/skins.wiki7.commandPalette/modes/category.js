...
function adaptCategoryItem( bareTitle ) {
	...
	return {
		id: 'wiki7-command-palette-item-category-' + bareTitle,
		...
		actions: [
			{
				id: 'view',
				label: mw.message( 'wiki7-command-palette-action-view' ).text(),
				icon: cdxIconArticle,
...
function adaptPageItem( apiPage ) {
	return {
		id: 'wiki7-command-palette-item-categorymember-' + apiPage.title,
...
function createCategoryMode( ApiConstructor ) {
	...
	function headerLabel( modeContext ) {
		...
		const root = mw.message( 'wiki7-command-palette-mode-category-breadcrumb-root' ).text();
	...
	return defineMode( {
		id: 'category',
		triggers: [ '/cat:', '#' ],
		label: mw.message( 'wiki7-command-palette-command-category-label' ).text(),
		description: mw.message( 'wiki7-command-palette-command-category-description' ).text(),
		placeholder: mw.message( 'wiki7-command-palette-mode-category-placeholder' ).text(),
		icon: cdxIconTag,
		compactResults: true,
		emptyState: {
			title: mw.message( 'wiki7-command-palette-mode-category-empty-title' ).text(),
			description: mw.message( 'wiki7-command-palette-mode-category-empty-description' ).text(),
			icon: cdxIconTag
		},
		noResults() {
			return {
				title: mw.message( 'wiki7-command-palette-mode-category-noresults-title' ).text(),
				description: mw.message( 'wiki7-command-palette-mode-category-noresults-description' ).text(),
				icon: cdxIconTag
			};
		},
		help: {
			description: 'wiki7-command-palette-mode-category-description-help'
		},
		getResults,
		onResultSelect,
		headerLabel
