...
function buildDetailPairs( info ) {
	...
	if ( sizeText ) {
		pairs.push( {
			key: 'size',
			label: mw.message( 'wiki7-command-palette-mode-file-detail-size' ).text(),
			value: sizeText
		} );
	}
	...
	if ( info.timestamp && info.user ) {
		pairs.push( {
			key: 'uploaded',
			label: mw.message( 'wiki7-command-palette-mode-file-detail-uploaded' ).text(),
	...
	if ( license ) {
		pairs.push( {
			key: 'license',
			label: mw.message( 'wiki7-command-palette-mode-file-detail-license' ).text(),
			value: license
		} );
	}
 ...
 * measurements against starwiki7.tools/Hull_B.
 *
...
function adaptListItem( page ) {
	...
	return {
		id: 'wiki7-command-palette-item-file-' + page.pageid,
		type: 'file',
...
function createFileMode( ApiConstructor ) {
	...
	return defineMode( {
		id: 'file',
		triggers: [ '/file:', '~' ],
		label: mw.message( 'wiki7-command-palette-command-file-label' ).text(),
		description: mw.message( 'wiki7-command-palette-command-file-description' ).text(),
		placeholder: mw.message( 'wiki7-command-palette-mode-file-placeholder' ).text(),
		icon: cdxIconImageGallery,
		layout: 'gallery',
		emptyState: {
			title: mw.message( 'wiki7-command-palette-mode-file-empty-title' ).text(),
			description: mw.message( 'wiki7-command-palette-mode-file-empty-description' ).text(),
			icon: cdxIconImageGallery
		},
		noResults() {
			return {
				title: mw.message( 'wiki7-command-palette-mode-file-noresults-title' ).text(),
				description: mw.message( 'wiki7-command-palette-mode-file-noresults-description' ).text(),
				icon: cdxIconImageGallery
			};
		},
		help: {
			description: 'wiki7-command-palette-mode-file-description-help'
		},
		getResults,
		getItemDetail,
		onResultSelect
