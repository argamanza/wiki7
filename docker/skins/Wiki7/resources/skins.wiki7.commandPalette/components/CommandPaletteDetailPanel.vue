<template>
	<div class="wiki7-command-palette-detail-panel" aria-live="polite">
		<!--
		...
		<command-palette-image
			v-if="detail.media"
			class="wiki7-command-palette-detail-panel__media"
		...
		<slot name="header">
			<div
				v-if="detail.header"
				class="wiki7-command-palette-detail-panel__header"
			>
				<div class="wiki7-command-palette-detail-panel__header-text">
					<div class="wiki7-command-palette-detail-panel__header-label">
						{{ detail.header.label }}
					</div>
					<div
						v-if="detail.header.description"
						class="wiki7-command-palette-detail-panel__header-description"
					>
				...
				<cdx-button
					...
					:title="copyTooltip"
					weight="quiet"
					class="wiki7-command-palette-detail-panel__header-copy"
					:class="{
						'wiki7-command-palette-detail-panel__header-copy--copied': copied
					}"
					@click="onCopyClick"
				>
		...
		<dl
			...
			class="wiki7-command-palette-detail-panel__pairs"
		>
			<div
				v-for="( pair, index ) in detail.pairs"
				:key="pair.key || index"
				class="wiki7-command-palette-detail-panel__pair"
			>
				<dt class="wiki7-command-palette-detail-panel__label">
					{{ pair.label }}
				</dt>
				<dd class="wiki7-command-palette-detail-panel__value">
...
const { CdxButton, CdxIcon } = mw.loader.require( 'skins.wiki7.commandPalette.codex' );
...
module.exports = exports = defineComponent( {
	...
	setup( props ) {
		...
		const copyAriaLabel = computed(
			() => mw.message( 'wiki7-command-palette-detail-copy-action' ).text()
		);
		const copyFeedback = computed(
			() => mw.message( 'wiki7-command-palette-detail-copy-feedback' ).text()
		);
...
.wiki7-command-palette-detail-panel {
