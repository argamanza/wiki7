<template>
	<div
		class="wiki7-command-palette-image"
		:class="rootClasses"
	>
		<img
			v-if="src && !isBroken"
			...
			:alt="alt"
			decoding="async"
			class="wiki7-command-palette-image__image"
			:class="imageClasses"
			@error="onError"
		>
		<span
			v-else
			class="wiki7-command-palette-image__placeholder"
		>
			<cdx-icon
				v-if="placeholderIcon"
				:icon="placeholderIcon"
				class="wiki7-command-palette-image__placeholder-icon"
			></cdx-icon>
		</span>
	</div>
</template>
...
module.exports = exports = defineComponent( {
	...
	setup( props ) {
		...
		const rootClasses = computed( () => props.aspectRatio ?
			[ `wiki7-command-palette-image--ratio-${ props.aspectRatio.replace( ':', '-' ) }` ] :
			[]
		);

		const imageClasses = computed( () => ( {
			[ `wiki7-command-palette-image__image--fit-${ props.objectFit }` ]: true
		} ) );
