<template>
	<div class="wiki7-notifications__panel">
		<header class="wiki7-notifications__header">
			<h2 class="wiki7-notifications__title">
				{{ msg.title }}
			</h2>
			<cdx-button
				class="wiki7-notifications__mark-all"
				weight="quiet"
		...
		<div class="wiki7-notifications__body">
			<div
				v-if="status === 'loading'"
				class="wiki7-notifications__skeleton"
				role="status"
				aria-busy="true"
			>
				<div
					v-for="n in 5"
					:key="n"
					class="wiki7-notifications__skeleton-item"
				>
					<span
						v-for="variant in [ 'title', 'meta' ]"
						:key="variant"
						class="wiki7-notifications__skeleton-line"
						:class="'wiki7-notifications__skeleton-line--' + variant"
					></span>
				</div>
			</div>
			<div
				v-else-if="status === 'error'"
				class="wiki7-notifications__error"
			>
				<p>{{ msg.error }}</p>
				<cdx-button
					class="wiki7-notifications__retry"
					@click="load"
				>
					{{ msg.retry }}
				</cdx-button>
			...
			>
				...
				>
					...
					<div
						v-else
						class="wiki7-notifications__empty"
					>
						{{ msg.empty }}
					</div>
				</cdx-tab>
		...
		<footer class="wiki7-notifications__footer">
			<a
				class="wiki7-notifications__see-all"
				:class="fakeQuietButtonClass"
				:href="urls.all"
			>{{ msg.seeAll }}</a>
			<a
				class="wiki7-notifications__prefs"
...
const { CdxButton, CdxIcon, CdxTab, CdxTabs } = mw.loader.require( 'skins.wiki7.notifications.codex' );
...
module.exports = exports = defineComponent( {
	...
	setup() {
		...
		const msg = {
			title: mw.message( 'wiki7-notifications-title' ).text(),
			markAll: mw.message( 'wiki7-notifications-mark-all-read' ).text(),
			empty: mw.message( 'wiki7-notifications-empty' ).text(),
			error: mw.message( 'wiki7-notifications-error' ).text(),
			retry: mw.message( 'wiki7-notifications-retry' ).text(),
			seeAll: mw.message( 'wiki7-notifications-see-all' ).text(),
			prefs: mw.message( 'wiki7-notifications-preferences' ).text(),
			tabAll: mw.message( 'wiki7-notifications-tab-all' ).text(),
			tabAlert: mw.message( 'wiki7-notifications-tab-alert' ).text(),
			tabNotice: mw.message( 'wiki7-notifications-tab-notice' ).text()
		};
