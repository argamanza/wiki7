<template>
	<li
		class="wiki7-notifications__item"
		:class="{ 'wiki7-notifications__item--unread': !item.read }"
		@click="onClick"
	>
		<!--
		...
		<a
			...
			class="wiki7-notifications__item-primary"
			:href="item.primaryUrl"
			:aria-label="plainHeader"
		></a>
		<div class="wiki7-notifications__item-content">
			<div class="wiki7-notifications__item-meta">
				<span
					v-if="item.categoryLabel"
					class="wiki7-notifications__item-category"
				>{{ item.categoryLabel }}</span>
				<span class="wiki7-notifications__item-time">{{ formattedTime }}</span>
			</div>
			<!-- eslint-disable vue/no-v-html -- header/body are messages Echo has already parsed and escaped. -->
			<div
				class="wiki7-notifications__item-header"
				v-html="item.header"
			></div>
			<div
				v-if="item.body"
				class="wiki7-notifications__item-body"
				v-html="item.body"
			></div>
			<!-- eslint-enable vue/no-v-html -->
			<div
				v-if="item.secondaryLinks.length > 0"
				class="wiki7-notifications__item-actions"
			>
				<a
					v-for="link in item.secondaryLinks"
					:key="link.url"
					class="wiki7-notifications__item-link"
