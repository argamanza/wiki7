<template>
	<div
		ref="viewport"
		class="wiki7-share-main">
		<template v-if="view === 'default'">
			<div class="wiki7-share-main__header">
				{{ i18n( 'wiki7-share' ) }}
			</div>

			<div
				class="wiki7-share-main__copy-link"
				:class="{ 'wiki7-share-main__copy-link--copied': copied }"
				@click="copyURL">
				<cdx-text-input
					id="wiki7-share-link"
					class="wiki7-share-main__copy-link__input"
					:model-value="pageURL"
					readonly></cdx-text-input>
				<cdx-button
					id="wiki7-share-copy-button"
					class="wiki7-share-main__copy-link__button"
					weight="quiet"
				...
				<div
					class="wiki7-share-main__copy-link__status"
					role="status"
					aria-live="polite">
					{{ copyStatusMessage }}
				</div>
			...
			<div
				...
				class="wiki7-share-main__url-controls">
				<cdx-button
					v-if="showCopyShortUrl"
					class="wiki7-share-main__copy-short-url"
					:class="{ 'wiki7-share-main__copy-short-url--copied': shortUrlCopied }"
					size="large"
		...
		<template v-else-if="view === 'qr'">
			<div class="wiki7-share-qr__header">
				<cdx-button
					weight="quiet"
					class="wiki7-share-qr__back"
					:aria-label="i18n( 'wiki7-share-back' )"
					@click="closeQrView">
					<cdx-icon :icon="cdxIconArrowPrevious"></cdx-icon>
				</cdx-button>
				<span class="wiki7-share-qr__title">
					{{ i18n( 'wiki7-share-qr-title' ) }}
				</span>
			</div>

			<div class="wiki7-share-qr__body">
				<div
					v-if="qrLoading"
					class="wiki7-share-qr__skeleton"
					role="status"
					:aria-label="i18n( 'wiki7-share-qr-title' )"></div>

				<template v-else-if="qrError">
					<p class="wiki7-share-qr__error">
						{{ i18n( 'wiki7-share-qr-error' ) }}
					</p>
					<cdx-button weight="primary" @click="retryQr">
						{{ i18n( 'wiki7-share-qr-retry' ) }}
					</cdx-button>
				</template>

				<template v-else>
					<img
						class="wiki7-share-qr__image"
						:src="qrCode"
						alt=""
						aria-hidden="true">
					<p class="wiki7-share-qr__hint">
						{{ i18n( 'wiki7-share-qr-scan-hint' ) }}
					</p>
					<p class="wiki7-share-qr__url">
						{{ shortUrlForQr }}
					</p>
				</template>
			</div>
...
const { CdxButton, CdxIcon, CdxTextInput } = mw.loader.require( 'skins.wiki7.share.codex' );
...
const SHARE_POPUP_WINDOW_NAME = 'wiki7-share-popup';
	...
	setup() {
		...
		const copyShortUrlButtonLabel = computed( () => (
			shortUrlCopied.value ?
				i18n( 'wiki7-share-short-url-copied' ) :
				i18n( 'wiki7-share-copy-short-url' )
		) );
		...
		async function writeToClipboard( text ) {
			const link = document.getElementById( 'wiki7-share-link' );
			try {
				if ( link ) {
					link.select();
				}
		...
		function copyShortUrl() {
			shortener.fetch( pageURL, { withQr: false } ).then( async () => {
				...
				if ( !url ) {
					...
					mw.notify( i18n( 'wiki7-share-short-url-error' ), {
						tag: 'wiki7-share',
						type: 'error'
					} );
					return;
		...
		const copyButtonAriaLabel = computed( () => (
			copied.value ?
				i18n( 'wiki7-share-copied' ) :
				i18n( 'wiki7-share-copy-link' )
		) );

		const copyStatusMessage = computed( () => (
			copied.value ? i18n( 'wiki7-share-copied' ) : ''
		) );
		...
		function shareServiceElementId( service, index ) {
			...
			return 'wiki7-share-service-' + suffix;
		}
		...
		const dialogEl = typeof document !== 'undefined' ?
			document.getElementById( 'wiki7-share-dialog' ) :
			null;
		...
		onMounted( () => {
			...
			nextTick( () => {
				const copyBtn = document.getElementById( 'wiki7-share-copy-button' );
				if ( copyBtn ) {
					copyBtn.focus();
				}
			} );
...
.wiki7-share-main__copy-link__status {
	.mixin-wiki7-screen-reader-only();
}

.wiki7-share-main__url-controls {
	display: flex;
	flex-wrap: wrap;
	gap: var( --space-xs );
	align-items: center;
...
.wiki7-share-main__social-option.cdx-button {
	// Brand-color tile: background comes from the per-service inline
	// style, icon goes white via the masked .wiki7-share-icon child.
	min-width: 0;
	color: #fff;
...
.wiki7-share-qr {
	...
	&__skeleton {
		...
		animation: wiki7-share-qr-pulse 1.4s ease-in-out infinite;
	}
...
@keyframes wiki7-share-qr-pulse {
	0%,
	100% {
		opacity: 1;
	}
