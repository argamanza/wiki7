 ...
 *
 * After mounting, fires `mw.hook('wiki7.preferences.register')` with a
 * `register( config )` callback that gadgets/extensions can use to add
 * sections and preferences at runtime.
 */
function initApp() {
	const mountPoint = document.getElementById( 'wiki7-preferences-content' );
	if ( !mountPoint ) {
		return;
	}
	...
	function register( registration ) {
		if (
			...
			mw.log.warn( 'wiki7.preferences.register: expected an object, got ' + typeof registration );
			return;
		}
	...
	const themeDefault = THEME_CONFIG_MAP[ serverConfig.wgWiki7ThemeDefault ] || 'os';
	...
	mw.hook( 'wiki7.preferences.register' ).fire( register );
}
