...
- **`getUserPermissionsErrors` hook restricting Wiki7Bot moves to NS_DRAFT** *(Docker — Phase 3b, from reviewer-pass orange #10 on 2026-06-13)* — the §6 ⑤ fix granted Wiki7Bot the `move` right globally. The router's "never cross namespaces" invariant means in practice the bot only ever moves within NS_DRAFT, but the MW-layer grant doesn't enforce that. A future bug in the router (or a malicious-bot-takeover scenario) could move a mainspace page. Add a `getUserPermissionsErrors` hook in `Wiki7ReviewGate` that denies `Wiki7Bot` (and any future bot in the `bot` group, if generalised) from moving pages whose origin OR destination is outside NS_DRAFT. Pattern: see ApprovedRevs's similar pattern for revision approvals. Effort: ~30 lines including a test.

- **Decide pre-Pattern-B: should `PageMoveComplete` trigger a review-pending notification?** *(Docker — Phase 3b, from reviewer-pass orange #10)* — Wiki7ReviewGate currently registers `PageSaveComplete` only, so a reviewer's mainspace rename `X → Y` (which leaves redirect-X) doesn't fire any notification. Two angles:
    1. The reviewer just made a deliberate edit so a notification would be noise.
    2. But Pattern A's auto-MovePage also fires `PageMoveComplete` and currently nobody sees it. Same noise/signal trade-off as reviewer's manual moves.
  Decision needed BEFORE Pattern B wires the surgical merger into save calls — if moves notify, the merger needs to understand that the notification is independent of the content change. Operator (Tzahi) to decide.
