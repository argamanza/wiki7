...
>
> **3a-pipeline shipped via PR #50** (2026-06-08, 14 tests, 4 spider correctness gaps closed). **3.5 shipped via PRs #51 + #52 + #53** (in-repo Wiki7ReviewGate extension + ApprovedRevs + Lockdown + Echo custom notification + Telegram dispatch + `$wgWiki7ReviewGateQuiet` site-wide kill-switch; ADR-0002 accepted with all 6 open questions resolved). **3a-content shipped via PRs #54 + #55** (one-time quiet-window flip-on, ran the pipeline → 113/113 created in 104.3s, all routed to NS_DRAFT or NS_TEMPLATE; flip-off restored normal notifications). Final 3a state: **102 \`Draft:*\` pages awaiting reviewer promotion on prod**; Telegram + Echo notifications confirmed working end-to-end; bulk import created zero notifications under the quiet flag; alarms clean throughout.
