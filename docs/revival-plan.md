...
- [ ] **Phase 3a R2 — pipeline finalization, multi-season, all-time** *(planned 2026-06-09; PR A = doc-only inventory; PR B = implementation + prod push)*. Re-runs the 3a-content push at all-time scope on a finalized pipeline. Gates the work behind the Phase 3.5 review gate + `$wgWiki7ReviewGateQuiet` flag, same recipe as the original 3a-content push.
    - **PR A (this round):** [`docs/research/0002-transfermarkt-data-surface.md`](research/0002-transfermarkt-data-surface.md) — comprehensive walkthrough of every TM page-type and field exposed for HBS across all eras; for each: presence in current pipeline (yes/partial/no), recommended action (implement / skip / defer-to-3b), reasoning. Includes the empirical TM season-availability floor (~1974/75–current, with gaps), curated-subset decisions, and 8 open questions for sign-off (season-identifier convention, cross-season aggregate strategy, translation strategy at 70× scale, ScraperAPI staging, …).
    - **PR B (after PR A merges):** spider/extractor changes closing every "implement" gap; multi-era HTML fixtures (2024/25 + 2015/16 + 1985/86) + pytest coverage; idempotency + resumability across seasons; LLM-assisted translation pass for the 70-season mappings file (nested `{he, src, confidence}` shape, backward-compat); graceful degradation on sparse historical data with a "what's missing" footer on historical-season pages; local all-time multi-season test against docker, then prod push under the quiet flag (gate + `WIKI_GATE_ENABLED=1`, recycle per [[wiki7-secret-rotation]], verify zero Telegram + Echo notifications during the run, flip quiet off after). **Drafts are NOT promoted** — reviewer promotion is 3b work.
    - **Doc updates in PR B:** mark 3a R2 done here with the prod-push numbers; groom `docs/phase-3b-backlog.md`; recipe for multi-season run in `docs/operational-bootstrap.md`; new `data/README.md`; new ADR(s) if architectural decisions emerged (translation strategy, "we don't import market-value history before 2003", etc.).
