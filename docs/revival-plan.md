...
- [ ] **Phase 3a R2 iteration-cycle (next session)**: per-season import + review + fix + repeat on local docker. Recommended order: 2024/25 first → walk backwards to ~2010/11 → review modern aggregates → jump to 1985/86 → walk forward → fill historical placeholders by hand. Each cycle: reset drafts → fresh translation (Wikidata-first, planned for next session) → import → review → fix templates / spiders / translations → repeat. Prod push (PR B step 11) happens once iteration cycle reaches v1 quality.
- [ ] **Human-review the auto-Hebrew translations** of the 102 imported drafts (in-wiki review queue at `Special:UnapprovedPages` + `Special:AllPages?namespace=3000`; promote via `Special:MovePage` per draft, or edit-then-promote when the auto-translation needs fixing). Mappings file is at `data/data_pipeline/output/2024/mappings.he.yaml` for batch corrections. *(Post-3a-R2: this becomes a per-season + bulk-by-namespace review across the all-time corpus — see PR A's §5.4 for the reviewer-queue scaling proposal that lands as Phase 3b tooling.)*
