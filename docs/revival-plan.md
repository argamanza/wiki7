...
- [ ] **Human-review the auto-Hebrew translations** of the 102 imported drafts (in-wiki review queue at `Special:UnapprovedPages` + `Special:AllPages?namespace=3000`; promote via `Special:MovePage` per draft, or edit-then-promote when the auto-translation needs fixing). Mappings file is at `data/data_pipeline/output/2024/mappings.he.yaml` for batch corrections.
