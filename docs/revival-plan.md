...
### Phase 2.5c — Pre-Phase-3 platform verification  *(✅ DONE 2026-06-07 — Round 1 + Round 2 complete)*
...
- **Round 1 ✅ DONE 2026-06-06 (post-#38 deploy validation + pre-2.5b baseline)** — PRs #41 + #42. ~15 matrix items executed (A1/A2/A5/A12/A13/A15-A18, B1/B4/B5/B11/B14/B15/B18, C1-C24 selected, D1/D2/D3, E14/E15, F2, G1/G3/G4/G5/G7, K4/K5 + PageSpeed K1/K2). **5 findings surfaced (2 security 🚨, 1 SEO ⚠️, 1 ops ⚠️, 1 IAM 🟢)** — captured in §Phase 2.5d above. **40+ confirmations recorded as ✅**, including end-to-end proof that PR #38's CloudFront-Viewer-Address → REMOTE_ADDR rewrite works in prod (rc_ip in `recentchanges` = real client IP, not a CloudFront edge). **Perf baseline (PageSpeed desktop 99 / mobile 92)** recorded as K6 snapshot for the 2.5b before/after comparison.
- **Round 2 ✅ DONE 2026-06-07 (post-2.5b, pre-Phase-3)** — the remaining ~85 matrix items end-to-end + re-verification of the six rows whose outcome was expected to shift after 2.5b (G2, G6, B14, C11, C13, C14) — all six confirmed green. **1 new finding (Finding 6 — KMS at ~$30/mo from 18 enabled customer-managed keys)** at severity 🟢 (cost-only, Phase 4 carry-over); zero functional regressions; ⏸️ items reduced to UI-click checks (Search Console, FB/LI/WA/TG previews, dashboard render, UptimeRobot, free-tier headroom, VE + Upload smoke). Full Round 2 outcomes table + Finding 6 detail in [`docs/phase-2.5c-platform-verification.md`](phase-2.5c-platform-verification.md) §Round 2.
...
- **Restore drill policy:** reuse the 2026-06-06 drill if < 30 days old at Round 2 execution time (saves 30 min); re-run otherwise. *(Reused 2026-06-07 — 1 day old.)*
- **Exit:** ✅ all 🔴 pass; all 🟡 either pass or have a tracked follow-up; all 🟢 either pass or are explicitly accepted; verification doc committed with outcomes. **🟢 Phase 3 unblocked.**
...
- **Local `argamanza` IAM gap for backup-vault read.** *Phase 2.5c Round 1 Finding 5:* the developer IAM principal lacks `backup:ListRecoveryPointsByBackupVault` / `backup:DescribeBackupVault` / `backup:DescribeRecoveryPoint`. Backups themselves are fine — the AWS Backup service role has the permissions; this is read-only verification from a dev laptop. Workaround: verify via the AWS console. Real fix: add the three permissions to the principal's inline policy. *Defer:* near-zero risk, blocks no work; revisit when the IAM principal is being maintained for another reason. *(Round 2 update: `list-recovery-points-by-backup-vault` now works from the local profile — at least one of the three perms is present; the `Describe*` variants were not retried this pass.)*
- **KMS orphan-key cleanup.** *Phase 2.5c Round 2 Finding 6 — ✅ RESOLVED 2026-06-07:* investigated 18 enabled customer-managed keys in `il-central-1`; confirmed 17 orphans (zero aliases, zero CFN tags, zero `Encrypt`/`Decrypt`/`GenerateDataKey` events in CloudTrail over 30 days, zero active grants, zero retained snapshots/recovery points referencing them) vs 1 live (the backup vault key, `990bd736-...`). All 17 scheduled for deletion (7-day pending window, actual deletion 2026-06-14). Storage billing stopped immediately on `PendingDeletion` transition. Expected drop: KMS daily ~$1.00 → ~$0.03; monthly ~$30 → ~$1, returning ADR-0001's KMS line to accuracy. *Remaining Phase 4 work:* (optional) add a CDK guardrail asserting `AWS::KMS::Key` count stays in a known range + a Cost Anomaly Detection monitor on the `wiki7-alarms` SNS topic, so the next time orphan keys accumulate the bill catches it before it grows.
...
> **Mid-Phase-3 re-shape (2026-06-08):** Phase 3a was split into **3a-pipeline** (bot account, spider fixes, gate-routed pipeline) and a deferred **3a-content** (the actual prod content push); a new **Phase 3.5** was inserted between them to build the bot-writes-must-pass-review platform. The first prod content arrived through the gate, not directly into mainspace. See [`docs/adr/0002-review-gate-architecture.md`](adr/0002-review-gate-architecture.md) + [`docs/research/0001-mediawiki-review-gate.md`](research/0001-mediawiki-review-gate.md) for the architecture, [`docs/phase-3b-backlog.md`](phase-3b-backlog.md) for the follow-up backlog this phase generated, and [`docs/operational-bootstrap.md`](operational-bootstrap.md) for the one-time post-deploy actions.
>
> **3a-pipeline shipped via PR #50** (2026-06-08, 14 tests, 4 spider correctness gaps closed). **3.5 shipped via PRs #51 + #52 + #53** (in-repo Wiki7ReviewGate extension + ApprovedRevs + Lockdown + Echo custom notification + Telegram dispatch + `$wgWiki7ReviewGateQuiet` site-wide kill-switch; ADR-0002 accepted with all 6 open questions resolved). **3a-content shipped via PRs #54 + #55** (one-time quiet-window flip-on, ran the pipeline → 113/113 created in 104.3s, all routed to NS_DRAFT or NS_TEMPLATE; flip-off restored normal notifications). Final 3a state at run time: **102 \`Draft:*\` pages on prod**; Telegram + Echo notifications confirmed working end-to-end; bulk import created zero notifications under the quiet flag; alarms clean throughout. *(Content subsequently wiped 2026-06-08 in the clean-slate teardown + redeploy — `cdk destroy` + fresh `cdk deploy`; prod returned to seed-only state. The 3a-content push will be re-done at all-time scope as part of Phase 3a R2; the original code/infra work done in 3a-pipeline + 3.5 + 3a-content stands.)*
>
> **Phase 3a R2 inserted (2026-06-09):** with the wiped prod content giving us a green-field re-run, 3a R2 finalises the pipeline before re-pushing: extends it from single-season (2024/25) to all-time, closes per-entity field gaps surfaced by a comprehensive TM-data-surface inventory, adds multi-era HTML fixtures + pytest coverage, idempotency + resumability across seasons, graceful degradation on sparse historical data, and an LLM-assisted translation pass at 70-season scale. PR A is the doc-only inventory + decisions ([`docs/research/0002-transfermarkt-data-surface.md`](research/0002-transfermarkt-data-surface.md)); PR B bundles implementation + tests + docs + the prod push. Same review-gate + quiet-flag recipe as the original 3a-content push.
>
> **Phase 3a R2 status (2026-06-10):** PR A merged (PR #61). PR B steps 1-10 done locally; step 11 (prod push) DEFERRED; step 12 (doc updates) is this commit. All-time pipeline ran end-to-end against local docker: **2,680 pages created, 0 failures, 7,893 ScraperAPI credits**. Review of the local artifacts surfaced four strategic findings (translation coverage 0.8% via English-Wikipedia-langlinks, duplicate pages on translation drift, Cargo template approval workflow gap, bulk review impractical for solo reviewer) that defer prod push in favour of an **iteration-cycle phase**: per-season import + review + fix + repeat on local docker, starting at 2024/25 and walking backwards. Full plan + Wikidata-overhaul research at [`docs/research/0003-translation-overhaul-plan.md`](research/0003-translation-overhaul-plan.md). Prod push happens after iteration cycle reaches v1 quality.

- [x] ~~Stand up the bot account and **run the full data pipeline end-to-end** against local → prod for the first time~~ — **DONE.** Wiki7Bot on prod in `bot` group; pipeline ran 2026-06-08 with `WIKI_GATE_ENABLED=1` against prod → 113 pages, 0 failures.
- [x] ~~Fix pipeline correctness gaps: match records missing `season`, brittle lineup/match-event extraction, empty club spiders (coaches / club transfers)~~ — **DONE (PR #50).** All 4 gaps closed empirically against HTML fixtures committed to `data/tests/fixtures/`; 95 → 113 pytest passing. The "lineup CSS" gap was a botched `name`→`name_english` rename; the "coaches empty" gap was a wrong URL path (`/trainer/` ["coach page"] → `/mitarbeiter/` ["staff page"]); the "transfers empty" gap was a multi-season HTML response + a dropped `table.items` class.
- [x] **Phase 3a R2 — pipeline finalization, multi-season, all-time** *(PR A merged + PR B steps 1-10 done 2026-06-09; PR B step 11 = prod push DEFERRED 2026-06-10; PR B step 12 = doc updates landed in this commit)*.
    - **PR A (PR #61 merged 2026-06-09):** [`docs/research/0002-transfermarkt-data-surface.md`](research/0002-transfermarkt-data-surface.md) — comprehensive walkthrough of every TM page-type and field exposed for HBS across all eras; per-entity inventory; empirical TM season-availability floor; 8 open questions resolved.
    - **PR B steps 1-10 (branch `phase-3a-r2/implementation`, 14 commits):** multi-era HTML fixtures (1985 + 2015 + 2024 + 1965/66 empty + AET sample), Pydantic + Cargo schema extensions (Player/Match/Transfer/Coach + new SeasonStanding + HeadToHead), existing-spider closures (match referee/halftime/AET/stadium, player foot/height/contract, records direction marker), two new spiders (`platzierungen` standings + `bilanz` head-to-head), trophy-derivation join (honours × per-season manager), honours-spider rewrite (TM HTML changed), season-identifier normalisation (slash format on human surfaces), translation refactor (Wikipedia → Claude chain, `WIKI7_ANTHROPIC_API_KEY` env var isolation, nested `{he, src, confidence}` shape), idempotency + resume + sparse-empty tolerance, always-emit season placeholders 1949/50→current, two new page types (Derbies + European campaign), AET marker detection (halbzeit slot reads "AET" not a score), NS_DRAFT bot READ permission fix, `_import_single_page` gate-routing fix (counts were misleading), full all-time test against local docker (2,680 pages, 0 failures, 7,893 ScraperAPI credits, 249/249 pytest).
    - **PR B step 11 (prod push) — DEFERRED 2026-06-10:** four strategic findings from step 10 review push prod push back behind an iteration-cycle phase. Findings: (a) translation coverage via English-Wikipedia-langlinks is only 0.8% so we're switching to Wikidata as the cross-language bridge; (b) duplicate-page problem from translation drift between runs needs a `tm_id → he_name` state file + MovePage on change; (c) Cargo template approval workflow needs documenting (Cargo tables don't materialise until reviewer approves `Template:Cargo/*`); (d) bulk all-time review is overwhelming for solo reviewer → switching to season-by-season iteration cycles. Plan in [`docs/research/0003-translation-overhaul-plan.md`](research/0003-translation-overhaul-plan.md).
    - **PR B step 12 (doc updates) — this commit:** Phase 3a R2 status here, `docs/phase-3b-backlog.md` groomed (closed items checked off + new items filed), `docs/operational-bootstrap.md` §8 added (multi-season recipe + Cargo approval workflow + WIKI7_ANTHROPIC_API_KEY pattern + iteration-cycle recipe), `data/README.md` added (pipeline architecture + new env vars + Wikipedia/Claude chain explanation). No new ADR — translation strategy ADR deferred until Wikidata path is implemented in the next session.
- [~] **Phase 3a R2 iteration-cycle — cycle 1 (2024/25): code shipped, awaiting reviewer walk (2026-06-10).** Per-season import + review + fix + repeat on local docker. Recommended order: 2024/25 first → walk backwards to ~2010/11 → review modern aggregates → jump to 1985/86 → walk forward → fill historical placeholders by hand. Each cycle: reset drafts → fresh translation (Wikidata-first) → import → review → fix templates / spiders / translations → repeat. Prod push (PR B step 11) happens once iteration cycle reaches v1 quality. Cycle 1 deliverables landed on `phase-3a-r2/implementation` (uncommitted at time of writing this entry — landed in next commit): (a) new `data_pipeline/wikidata_lookup.py` Wikidata-first translation chain with retry-budget + maxlag + WARNING-visible failure paths; (b) match spider captures `tm_player_id` + slug-derived full names from `<a href>` on lineups + goal/sub/card/penalty events (fixes the surname-only-in-lineup data-quality issue that was duplicating names corpus entries); (c) match report template wraps every player mention in `{{#ifexist:NAME|[[NAME]]|{{#ifexist:Draft:NAME|[[Draft:NAME|NAME]]|NAME}}}}` so HBS players link to mainspace/draft, opposing players render as plain text, and historic match mentions become live links retroactively when an opponent later joins HBS; (d) data-loss audit shipping for every external surface (Wikidata retry-budget bumped 2→5 retries, MW `bot` group granted `noratelimit` explicitly since MW 1.45 default lacks it, Wikipedia langlinks failures promoted DEBUG→WARNING). 283/283 pytest. Empirical 2024/25 coverage: 117/117 pages, 0 failures across all surfaces, 74.7% canonical-source name coverage on the cleaner corpus.
- [ ] **Human-review of the 103 imported 2024/25 drafts** (`Special:AllPages?namespace=3000` — drafts aren't ApprovedRevs-gated, the queue is just NS_DRAFT; templates need approval via `Special:ApprovedRevs?show=unapproved` first; promote via `Special:MovePage` per draft, or edit-then-promote when the auto-translation needs fixing). Mappings file at `data/data_pipeline/output/2024/mappings.he.yaml` for batch corrections (manual entries with `src: manual` survive re-runs).
...
- [ ] **Social-card cross-platform review — re-run when content exists** *(recorded 2026-06-07 after Phase 2.5c Round 2)*. The seed-state checks (FB Sharing Debugger, LinkedIn Post Inspector, opengraph.xyz, WhatsApp, Telegram, Google `site:`) all rendered the homepage card correctly, but some observations are only meaningful against article pages (multiple page-types, real content lengths). Re-run all six previews against at least 2-3 representative content pages (a player, a club page, a match page) once Phase 3 lands content, and decide on each finding:
    - **Telegram brand duplication** *(new — only observed on Telegram, no other client)*: `og:site_name = "ויקישבע"` and `og:title = "ויקישבע - אנציקלופדיית הפועל באר שבע"` both appear in the Telegram card, so the brand reads twice. WhatsApp/Facebook/LinkedIn detect the prefix overlap and collapse it; Telegram doesn't. **Two fix options:** (a) drop the brand from the homepage's `og:title` (leave the brand to `og:site_name` only); homepage og:title becomes just `אנציקלופדיית הפועל באר שבע` while article pages stay `<title> - ויקישבע` and behave the same in Telegram (article title ≠ site name → no duplication). (b) accept the Telegram quirk — card still perfectly readable. **Re-test on article pages first**: if articles trigger the same duplication shape, the fix is worth it; if only the homepage does, (b) may be enough. Touches the `WikiSEOPreAddMetadata` + `BeforePageDisplay` hooks in `docker/LocalSettings.php`.
    - **Google search-result title shortening** *(observed 2026-06-07 in `site:wiki7.co.il`)*: Google's snippet showed just `ויקישבע` rather than the full brand-augmented title. Could be Google compressing for display, or a hint that the snippet algorithm prefers `og:site_name` over `og:title` on landing pages. Verify once articles are indexed: if articles also show as just `ויקישבע` instead of `<article> - ויקישבע`, that's a real signal-loss problem and we should investigate (possibly via per-page `{{#seo:title=}}` overrides).
    - **LinkedIn / opengraph.xyz "description is short (~61 chars; optimal 110-160)"** *(re-noted 2026-06-07)*: same root cause as the existing description-length item above; subsumed by that one. Listed here for traceability.
    - **Facebook Sharing Debugger Response Code 206 (Partial Content)** *(observed 2026-06-07)*: FB's scraper apparently sends a `Range` header and Apache honors it with a 206 response. The card preview rendered correctly, so functional. Worth a one-line check that this doesn't degrade as Phase 3 introduces real content (larger HTML responses might amplify any Range-handling quirk). **Decision: monitor, don't fix preemptively.**
