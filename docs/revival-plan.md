...
- **KMS orphan-key cleanup.** *Phase 2.5c Round 2 Finding 6:* `il-central-1` has 18 enabled customer-managed KMS keys, costing ~$1/day in storage + per-API charges (~$30/mo), against an ADR-0001 expected `~$1-2/mo`. Most are almost certainly leftover from prior teardown cycles + multi-version CDK deploys where the key's `removalPolicy` wasn't `DESTROY`. *Fix:* inventory each via `aws kms list-aliases` + `describe-key` + CloudTrail "last use", then `aws kms schedule-key-deletion --pending-window-in-days 7` for confirmed orphans. *Defer:* pure cost cleanup, no functional impact; the ADR's KMS line is the one being violated, not the security posture.
