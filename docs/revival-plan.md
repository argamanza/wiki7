...
**Phase 2.5b — actual edge caching of MW HTML** *(planned, immediately after Phase 2.5d validates on prod; before Phase 3)*
...
- **Scope sketch (will research before implementing):**
    ...
    - **`$wgCdnMaxAge` design call (explicit).** MW's default is 18000s = 5 h. This is the staleness window for an edit if we don't invalidate. Three honest options: (a) keep the 5h default + skip invalidation entirely (simplest, fine for a low-edit wiki, awful during a Phase-3 pipeline run where the data bot writes many pages back-to-back and readers would see hours-old state); (b) shorten `$wgCdnMaxAge` to e.g. 600s (10 min) — bounds the worst case, reduces cache-hit benefit proportionally; (c) keep the 5h default + add edit-driven `CreateInvalidation`. Decide based on the edit-cadence we want Phase 3 to feel like, not on what's easiest to wire.
    - Edit invalidation: either CloudFront `CreateInvalidation` on edit (Lambda + IAM, low volume on a 1-author wiki = effectively free — 1000 paths/month are included in the AWS Free Tier), or skip and rely on `$wgCdnMaxAge`. Coupled to the design call above.
    ...
    - **Roll-in from Round 1 — Finding 3 (Schema.org `@type` casing).** While touching WikiSEO config for the SEO-side bits of edge caching (cache-key cookies, surrogate Cache-Control, etc.), fix the lowercase `"website"`/`"article"` emitted in JSON-LD. Either find a separate WikiSEO key for Schema.org @type vs OG type, or emit `WebSite`/`Article` and verify OG accepts the casing (per OG spec it's case-insensitive in practice).
- **Validation:**
    - Synthetic anon GETs vs logged-in GETs through CloudFront; observe the `Age:` header crosses zero on cache hits and stays absent for logged-in requests.
    - "Edit page → expected staleness window → invalidation behaves (or `$wgCdnMaxAge` ages out)" smoke test.
    - **`$wgRateLimits` sanity check.** This is the moment real per-user IPs first actually apply at the rate-limit layer (pre-#38 every viewer looked like a CloudFront edge IP, so per-IP limits were nonsense). MW's defaults are conservative; confirm no default would block a single legit editor doing a pipeline-cleanup spree (`edit`, `move`, `linkpurge`, `purge` groups). Adjust in `LocalSettings.php` if needed — Phase 3 data-pipeline runs are the use case to size for.
- **References:** `cdk/lib/cloudfront-stack.ts` (default behavior + cache policies), `docker/LocalSettings.php` (the `$wgUseCdn = true` block has the inline note pointing here), [Manual:CloudFront](https://www.mediawiki.org/wiki/Manual:CloudFront), [Manual:Performance tuning](https://www.mediawiki.org/wiki/Manual:Performance_tuning), [Manual:$wgRateLimits](https://www.mediawiki.org/wiki/Manual:%24wgRateLimits).
- **Exit:**
